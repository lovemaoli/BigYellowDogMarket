...
public class   UserController
{
    ...
    public String login(HttpServletRequest request, @RequestParam String sNum,
        ...
        User user = service.Login(sNum, Password, isAdmin);
        if (user == null)
        {
...
//    }
    ...
    public String insertGoods(@RequestParam(required = false) MultipartFile image,
        ...
        if(!image.isEmpty())
        {
            ...
            goods = new Goods(Name, Kind, Price, Number, random, Comment);
        }
        else
        {
            goods = new Goods(Name, Kind, Price, Number, null, Comment);
        }
    ...
    public String deleteMyProduct(HttpServletRequest request, Model model, @RequestParam String GID)
    {
        ...
        Goods goods = goodsService.SelectGoodsByPK(GID);

        service.RemoveSaleGoods(user.getUID(), goods);
    ...
    {
        Goods goods = (Goods) request.getSession().getAttribute("marketGoods");
        User user = (User) request.getSession().getAttribute("marketUser");
    ...
    {
        User user = (User) request.getSession().getAttribute("marketUser");
        PurchaseRecordLists purchaseRecordLists = service.ShowPRecord(user,true,true);

        List<PurchaseRecord> myList = purchaseRecordLists.getPurchaseRecordList();
    ...
    {
        User user = (User) request.getSession().getAttribute("marketUser");
        PurchaseRecordLists purchaseRecordLists = service.ShowPRecord(user,false,false);
    ...
    {
        User user = (User) request.getSession().getAttribute("marketUser");
        PurchaseRecordLists purchaseRecordLists = service.ShowPRecord(user,true,false);
    ...
    {
        User user = (User) request.getSession().getAttribute("marketUser");
        SaleRecordLists saleRecordLists = service.ShowSRecord(user,true,true);

        List<SaleRecord> myList = saleRecordLists.getSaleRecordList();
    ...
    {
        User user = (User) request.getSession().getAttribute("marketUser");
        SaleRecordLists saleRecordLists = service.ShowSRecord(user,false,false);
    ...
    {
        User user = (User) request.getSession().getAttribute("marketUser");
        SaleRecordLists saleRecordLists = service.ShowSRecord(user,true,false);
    ...
    public String goToMyCart(HttpServletRequest request, Model model)
    {
        ...
        ShoppingCarts myCart = service.ShowCar(user);
        List<Goods> myCartList = myCart.getGoods();

        for(Goods temp : myCartList)
        {
            User tempUser = service.WhoseGoods(temp.getGID());
    ...
    public ModelAndView addCar(HttpServletRequest request, Model model, @RequestParam String GID)
    {
        ...
        User user = (User) request.getSession().getAttribute("marketUser");
        Goods goods = goodsService.SelectGoodsByPK(GID);
        User seller = service.WhoseGoods(GID);
